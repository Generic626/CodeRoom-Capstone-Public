...
const Notebook = () => {
  ...
  const handleCloneClick = async () => {
    // If the author hasn't shared the notebook, display error
    if (sharedNotebookList.length == 0) {
      setError({
        errorText: "Please shared this notebook first before cloning",
        isError: true,
      });
    }
    // else prepare to call clone notebook api
    await await axios
      .post(
        `http://localhost:5001/api/notebook/clone/${notebookId}`,
        {},
        {
  ...
  }, [socket, notebookId, requestHeaders, setSharedNotebookList]);
  ...
  //     socket.emit("save-notebook", { code, title, readOnly });
  //   }, 2000);

  //   return () => {
  //     clearInterval(interval);
  //   };
  // }, [socket, code, title, readOnly]);
